...
//    $comment = SingleQryFld("SELECT to_char(CONTENT_COMMENT) FROM MST_COMMENT WHERE TO_CHAR(COMMENT_DATE, 'MM/DD/YYYY')='$yesterday' AND COMMENT_TYPE = 'ASSEMBLY' AND PROJECT_NO = 'W15032'", $conn);
//    $report = SingleQryFld("SELECT COMMENT_SIGN FROM MST_COMMENT WHERE TO_CHAR(COMMENT_DATE, 'MM/DD/YYYY')='$yesterday' AND COMMENT_TYPE = 'ASSEMBLY' AND PROJECT_NO = 'W15032'", $conn);
    $header = '<table>'
            . '<tr><th style="text-align:center;" colspan=17><b><u>LAPORAN PROGRESS BY ASSEMBLY W15032  (CONVEYOR PT.    SEMEN INDONESIA) TANGGAL ' . "$date" . '</u></b></th></tr>'
    ...
    $content = "";
    $projectSql = "SELECT DISTINCT PROJECT_NAME_OLD,CLIENT_INIT,PROJECT_NAME_NEW, PROJECT_DESC FROM VW_PROJ_INFO "
            . "WHERE PROJECT_TYP = 'STRUCTURE' AND PROJECT_NO = 'W15032' ORDER BY CLIENT_INIT,PROJECT_NAME_NEW";
    ...
    while ($row1 = oci_fetch_array($projectParse)) {
        ...
        $content.='<tr><td colspan="22" style="background-color:#fd9064;">' . $row1['CLIENT_INIT'] . ' - ' . $row1['PROJECT_NAME_NEW'] . " <i>($row1[PROJECT_DESC])<i>" . '</td></tr>';
...
//    exit();
    ...
    $m = mail($recepient, 'LAPORAN PROGRESS BY ASSEMBLY W15032  (CONVEYOR PT.    SEMEN INDONESIA)', $qwe, $mailheaders);
    $IDEMAIL = "email-" . date("d/m/Y");
    if ($m) {
        $insertEmailSql = "INSERT INTO EMAIL_NOTIFICATION (ID_EMAIL, RECEIVER, SENT_DATE, STATUS, EMAIL_TYPE) VALUES('$IDEMAIL', '$recepient', SYSDATE, 'SUCCESS', 'W15032')";
        ...
        $IDEMAIL = "email-" . date("d/m/Y");
        $insertEmailSql = "INSERT INTO EMAIL_NOTIFICATION (ID_EMAIL, RECEIVER, SENT_DATE, STATUS, EMAIL_TYPE) VALUES('$IDEMAIL', '$recepient', SYSDATE, 'FAILSS', 'W15032')";
        $insertEmailParse = oci_parse($conn, $insertEmailSql);
        $insertEmail = oci_execute($insertEmailParse);
        if ($insertEmail) {
            oci_commit($conn);
...
if ($hour > 12 && $hour < 24) {
    $IDEMAIL = "email-" . date("d/m/Y");
    $cekEmailSql1 = "SELECT COUNT(*) JUMLAH FROM EMAIL_NOTIFICATION WHERE ID_EMAIL = '$IDEMAIL' AND STATUS = 'SUCCESS' AND EMAIL_TYPE = 'W15032'";
    $result1 = SingleQryFld($cekEmailSql1, $conn);
    if ($result1 == "0") {
        DailyFabrication();
    } else
        echo "email W15032 sudah terkirim" . "<br/>";
}
else {
