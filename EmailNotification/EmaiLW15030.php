...
//    $comment = SingleQryFld("SELECT to_char(CONTENT_COMMENT) FROM MST_COMMENT WHERE TO_CHAR(COMMENT_DATE, 'MM/DD/YYYY')='$yesterday' AND COMMENT_TYPE = 'ASSEMBLY' AND PROJECT_NO = 'W15030'", $conn);
//    $report = SingleQryFld("SELECT COMMENT_SIGN FROM MST_COMMENT WHERE TO_CHAR(COMMENT_DATE, 'MM/DD/YYYY')='$yesterday' AND COMMENT_TYPE = 'ASSEMBLY' AND PROJECT_NO = 'W15030'", $conn);
    $header = '<table>'
            . '<tr><th style="text-align:center;" colspan=17><b><u>LAPORAN PROGRESS BY ASSEMBLY W15030  (PT. WIJAYA KARYA & PELINDO) TANGGAL ' . $date . '</u></b></th></tr>'
    ...
    $projectSql = "SELECT DISTINCT PROJECT_NAME_OLD,CLIENT_INIT,PROJECT_NAME_NEW FROM VW_PROJ_INFO "
            . "WHERE PROJECT_TYP = 'STRUCTURE' AND PROJECT_NO = 'W15030' ORDER BY CLIENT_INIT,PROJECT_NAME_NEW";
...
//      exit();
    ...
    $m = mail($recepient, 'LAPORAN PROGRESS BY ASSEMBLY W15030 (PT. WIJAYA KARYA & PELINDO)', $qwe, $mailheaders);
    $IDEMAIL = "email-" . date("d/m/Y");
    if ($m) {
        $insertEmailSql = "INSERT INTO EMAIL_NOTIFICATION (ID_EMAIL, RECEIVER, SENT_DATE, STATUS, EMAIL_TYPE) VALUES('$IDEMAIL', '$recepient', SYSDATE, 'SUCCESS', 'W15030')";
        ...
        $IDEMAIL = "email-" . date("d/m/Y");
        $insertEmailSql = "INSERT INTO EMAIL_NOTIFICATION (ID_EMAIL, RECEIVER, SENT_DATE, STATUS, EMAIL_TYPE) VALUES('$IDEMAIL', '$recepient', SYSDATE, 'FAILSS', 'W15030')";
        $insertEmailParse = oci_parse($conn, $insertEmailSql);
        $insertEmail = oci_execute($insertEmailParse);
        if ($insertEmail) {
            oci_commit($conn);
...
if ($hour > 12 && $hour < 24) {
    $IDEMAIL = "email-" . date("d/m/Y");
    $cekEmailSql1 = "SELECT COUNT(*) JUMLAH FROM EMAIL_NOTIFICATION WHERE ID_EMAIL = '$IDEMAIL' AND STATUS = 'SUCCESS' AND EMAIL_TYPE = 'W15030'";
    $result1 = SingleQryFld($cekEmailSql1, $conn);
    if ($result1 == "0") {
        DailyFabrication();
    } else
        echo "email W15030 sudah terkirim" . "<br/>";
}
else {
