...
function DailyFabrication() {
    ...
    $comment = SingleQryFld("SELECT to_char(CONTENT_COMMENT) FROM MST_COMMENT WHERE TO_CHAR(COMMENT_DATE, 'MM/DD/YYYY')='$yesterday' AND COMMENT_TYPE = 'ASSEMBLY' AND PROJECT_NO = 'W14026'", $conn);
    $report = SingleQryFld("SELECT COMMENT_SIGN FROM MST_COMMENT WHERE TO_CHAR(COMMENT_DATE, 'MM/DD/YYYY')='$yesterday' AND COMMENT_TYPE = 'ASSEMBLY' AND PROJECT_NO = 'W14026'", $conn);
    $header = '<table>'
    . '<tr><th style="text-align:center;" colspan=17><b><u>LAPORAN PROGRESS BY ASSEMBLY W14026 (PABRIK GULA DOMPU ) TANGGAL ' . $date . '</u></b></th></tr>'
    ...
    . "WHERE PROJECT_TYP = 'STRUCTURE' AND PROJECT_NO = 'W14026' ORDER BY CLIENT_INIT,PROJECT_NAME_NEW";
    ...
    $m = mail($recepient, 'LAPORAN PROGRESS BY ASSEMBLY W14026 (PABRIK GULA DOMPU)', $qwe, $mailheaders);
    $IDEMAIL = "email-" . date("d/m/Y");
    if ($m) {
        $insertEmailSql = "INSERT INTO EMAIL_NOTIFICATION (ID_EMAIL, RECEIVER, SENT_DATE, STATUS, EMAIL_TYPE) VALUES('$IDEMAIL', '$recepient', SYSDATE, 'SUCCESS', 'W14026')";
        ...
        $IDEMAIL = "email-" . date("d/m/Y");
        $insertEmailSql = "INSERT INTO EMAIL_NOTIFICATION (ID_EMAIL, RECEIVER, SENT_DATE, STATUS, EMAIL_TYPE) VALUES('$IDEMAIL', '$recepient', SYSDATE, 'FAILSS', 'W14026')";
        $insertEmailParse = oci_parse($conn, $insertEmailSql);
        $insertEmail = oci_execute($insertEmailParse);
        if ($insertEmail) {
            oci_commit($conn);
...
if ($hour > 12 && $hour < 24) {
    $IDEMAIL = "email-" . date("d/m/Y");
    $cekEmailSql1 = "SELECT COUNT(*) JUMLAH FROM EMAIL_NOTIFICATION WHERE ID_EMAIL = '$IDEMAIL' AND STATUS = 'SUCCESS' AND EMAIL_TYPE = 'W14026'";
    $result1 = SingleQryFld($cekEmailSql1, $conn);
    if ($result1 == "0") {
        DailyFabrication();
    } else {
        echo "email W14026 sudah terkirim" . "<br/>";
    }
} else {
