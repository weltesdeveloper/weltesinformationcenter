...
function DailyFabrication() {
    ...
    $header = '<table>'
            . '<tr><th style="text-align:center;" colspan=17><b><u>LAPORAN PROGRESS BY ASSEMBLY W-IGG (INDUSTRI GULA GLENMORE) TANGGAL ' . $date . '</u></b></th></tr>'
    ...
    $projectSql = "SELECT DISTINCT PROJECT_NAME_OLD,CLIENT_INIT,PROJECT_NAME_NEW FROM VW_PROJ_INFO "
            . "WHERE PROJECT_TYP = 'STRUCTURE' AND PROJECT_NO = 'W-IGG' ORDER BY CLIENT_INIT,PROJECT_NAME_NEW";
...
//    exit();
    ...
    if ($m) {
        $insertEmailSql = "INSERT INTO EMAIL_NOTIFICATION (ID_EMAIL, RECEIVER, SENT_DATE, STATUS, EMAIL_TYPE) VALUES('$IDEMAIL', '$recepient', SYSDATE, 'SUCCESS', 'WIGG')";
        ...
        $IDEMAIL = "email-" . date("d/m/Y");
        $insertEmailSql = "INSERT INTO EMAIL_NOTIFICATION (ID_EMAIL, RECEIVER, SENT_DATE, STATUS, EMAIL_TYPE) VALUES('$IDEMAIL', '$recepient', SYSDATE, 'FAILSS', 'WIGG')";
        $insertEmailParse = oci_parse($conn, $insertEmailSql);
        $insertEmail = oci_execute($insertEmailParse);
        if ($insertEmail) {
            oci_commit($conn);
...
if ($hour > 12 && $hour < 24) {
    $IDEMAIL = "email-" . date("d/m/Y");
    $cekEmailSql1 = "SELECT COUNT(*) JUMLAH FROM EMAIL_NOTIFICATION WHERE ID_EMAIL = '$IDEMAIL' AND STATUS = 'SUCCESS' AND EMAIL_TYPE = 'WIGG'";
