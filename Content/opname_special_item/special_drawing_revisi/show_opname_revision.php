...
<div class="row">
    <table class="table table-striped table-bordered table-condensed" id="table-input">
        ...
        <tbody>
            <?php
            $sql = "SELECT * FROM VW_INFO_OPNAME_SI WHERE OPNAME_ID = '$_POST[opname_id]' ORDER BY HEAD_MARK ASC";
            ...
            while ($row = oci_fetch_array($parse)) {
                ...
                <tr id="rowtarget<?php echo "$i"; ?>">
                    ...
                    <td class="text-center" style="width: 100px;">
                        <input type="number" id="prosentase<?php echo "$i"; ?>" class="form-control" 
                               onchange="InputNumber('<?php echo $i; ?>', 'prosentase')" value="<?php echo $row['PROCEN_WEIGHT']; ?>" 
                               style="width: 100px;"
                               <?php
                               if ($OPNAME_STATUS == "CLOSE") {
                                   echo "disabled";
                    ...
                    <td class="text-center" style="width: 100px;">
                        <input type="text" id="remark<?php echo "$i"; ?>" class="form-control" value="<?php echo $row['REMARK']; ?>" 
                               style="width: 100px;"
                               <?php
                               if ($OPNAME_STATUS == "CLOSE") {
                                   echo "disabled";
...
//            }
    ...
    function SubmitData() {
        ...
        for (var x = 0; x < rows.length; x++) {
            ...
            weight.push($(rows[x]).find("td:eq(3)").text().trim());
            qty.push($(rows[x]).find("td:eq(4)").find("input").val());
            price.push($(rows[x]).find("td:eq(5)").find("input").val());
            prosentase.push($(rows[x]).find("td:eq(6)").find("input").val());
            remark_item.push($(rows[x]).find("td:eq(7)").find("input").val());
        }
...
//        } else {
        ...
        if (cf == true) {
            $.ajax({
                type: 'POST',
                url: "special_drawing_revisi/submit_data.php",
                data: sentReq,
    ...
    function InputNumber(index, type) {
        switch (type) {
            ...
            case "price":
                ...
                break;
            case "prosentase":
                var prosentase = parseFloat($('#prosentase' + index).val());
                if (prosentase < 0 || isNaN(prosentase) || prosentase == "") {
                    $('#prosentase' + index).val(0)
                }
                break;
        }
    }
