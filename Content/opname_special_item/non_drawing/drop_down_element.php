...
switch ($_POST['action']) {
    case "change_job":
        $array_subjob = array();
        $job = $_POST['job'];
        $subjobSql = "SELECT DISTINCT PROJECT_NAME_OLD, PROJECT_NAME_NEW FROM VW_PROJ_INFO "
    ...
    case "change_subjob":
        ...
        $job = $_POST['job'];
        $subjob = $_POST['subjob'];
        $subcontSql = "SELECT DISTINCT SUBCONT_ID FROM SUBCONTRACTOR WHERE SUBCONT_ACTUAL = 'ACTIVE' AND SUBCONT_STATUS = 'OUTSOURCE' ORDER BY SUBCONT_ID";
    ...
    case "change_periode":
        ...
        $OPNAME_IDX = "$job-$subjobInit-$subcontInit-$periode-$tanggal-SP";
