    ...
    <table class="table table-striped table-bordered" id="table-input">
        ...
        <tbody>
            ...
            while ($row = oci_fetch_array($parse)) {
                ...
                <tr id="rowtarget<?php echo "$i"; ?>">
                    ...
                    <td class="text-center">
                        <div id="totalprice<?php echo "$i"; ?>">
                            <?php
                            echo number_format($row['TOTAL_QTY'] * $row['WEIGHT'] * $row['OPN_PRICE']*$row['PROCEN_WEIGHT']/100, 2);
                            ?>
                        </div>
                    </td>
                </tr>
                <?php
                $i++;
                $total_price += ($row['TOTAL_QTY'] * $row['WEIGHT'] * $row['OPN_PRICE']*$row['PROCEN_WEIGHT']/100);
            }
            ?>
        </tbody>
        <tfoot>
...
<script>
    ...
    function Print() {
        ...
        var url = "special_drawing_print/print_opname.php?opname_id=" + opname_id;
