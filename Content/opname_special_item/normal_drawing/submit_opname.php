...
$opnamepersen = $_POST['opnamepersen'];
...
if ($cekMstOpname == 0) {
    $insertMstOpnameSql = "INSERT INTO MST_OPNAME(OPNAME_ID, PROJECT_NO, SUBCONT_ID, OPN_SYS_DATE, OPN_ACT_DATE, OPN_PERIOD, PROJECT_NAME, OPN_REQ_SIGN, OPN_STATUS) "
            . "VALUES('$opname_id', '$job', '$subcont', SYSDATE, TO_DATE('$tanggal_opname', 'DD/MM/YYYY'), '$periode', '$subjob', '$username', 'OPEN')";
    ...
    if ($insertMstOpname) {
        ...
        for ($index = 0; $index < sizeof($headmark); $index++) {
            $insertDtlOpnameSql = "INSERT INTO DTL_OPNAME(HEAD_MARK, TOTAL_QTY, OPN_PRICE, OPN_REQ_SIGN, PROJECT_NAME, OPN_SYS_DATE, OPN_ACT_DATE, OPNAME_ID, PROCEN_WEIGHT) "
                    . "VALUES('$headmark[$index]', '$opnameqty[$index]', '$opnameprice[$index]', '$username', '$subjob', SYSDATE, TO_DATE('$tanggal_opname', 'DD/MM/YYYY'), '$opname_id', '$opnamepersen[$index]')";
