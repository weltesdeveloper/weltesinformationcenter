...
<!-- <body> -->
  ...
  if($_POST['action'] == 'show')
  {
    ...
    WHERE PROJECT_NAME= '{$_POST["ProjNme"]}' AND HEAD_MARK "
    . "IN(SELECT DISTINCT(HEAD_MARK) FROM VW_OPN_INFO WHERE PROJECT_NAME=PNT.PROJECT_NAME AND (PAINT_QC_PASS <> OPN_BLS_QTY OR PAINT_QC_PASS <> OPN_PNT_QTY))  ORDER BY HEAD_MARK,ID";
      ...
      <!-- Modal -->
        <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
          <div class="modal-dialog modal-lg" style="width:800px;">
            <div class="modal-content" id="DetailHM_opn" >                          
            </div>
          </div>
        </div>
    <!-- </table> -->
...
<script type="text/javascript">
    ...
    function getHM_opn(HM,ID,SURF,FAB_PASS_QTY,SUBCONT) {
      $("#DetailHM_opn").empty();
      $.get('update_painting_opn.php',
          {
              HM: HM,
              ID: ID,
              SURF: SURF,
              FAB_PASS: FAB_PASS_QTY,
              SUBCONT: SUBCONT
          },
          function(res){
              $("#DetailHM_opn").html(res);
          }
      );
    }
