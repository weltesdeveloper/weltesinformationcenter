            ...
            while ($row = oci_fetch_array($finalOpnameParse)) {
                ...
                <tr id="rowsource<?php echo "$i"; ?>">
                    ...
                    <td class="text-center" style="vertical-align: middle;">
                        <?php
                        if ($row['COMP_TYPE'] != 'KINGCROSS' && $row['QTY_OPN'] == $row['QCPASS']) {
                            ?>
                            <button type='button' onclick="AddItem('<?php echo "$i"; ?>')" class="btn btn-primary" disabled>ADD</button>
                            <?php
                        } else {
                            ?>
...
<script>
    ...
    function AddItem(index) {
        ...
        var newTargetRow = table_target.fnAddData([
            ...
            qcdatesource,
            opnamesource,
            "<input type=number id='opnameqty" + counteradd + "' min=0 max=" + remaining + " value=" + remaining + " onchange=CekValidQty('" + counteradd + "') style='width:80px;'>",
    ...
    function SubmitOpname() {
        ...
        if (periode == "") {
            ...
            for (var x = 0; x < rows.length; x++)
            {
                ...
                opnameqty.push($(rows[x]).find("td:eq(6)").find('input').val());
                opnameprice.push($(rows[x]).find("td:eq(7)").find('input').val());
            }
            var sentReq = {
                job: job,
                subjob: subjob,
            ...
            if (cf == true) {
                $.ajax({
                    type: 'POST',
                    url: "divpages/submit_opname.php",
                    data: sentReq,
