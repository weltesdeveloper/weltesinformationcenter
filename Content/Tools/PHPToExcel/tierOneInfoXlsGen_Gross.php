...
$tierOneSql = "SELECT STOV.PROJECTNAME PROJECTNAME, "
        . "STOV.GROSSWEIGHT GROSSWEIGHT, "
        . "STOV.TOTALAVGEGRPROGRESS PROGRESSGROSS "
...
$objPHPExcel->setActiveSheetIndex(0)
        ->mergeCells('B1:F1')
        ->setCellValue('B1', 'SITE ERECTION PROJECT SUMMARY REPORT FOR GROSS WEIGHT');
...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D4', 'TOTAL GROSS WEIGHT (Kg)')
        ->setCellValue('E4', 'PROGRESS GROSS WEIGHT  (Kg)')
...
while ($row = oci_fetch_array($tierOneParse)) {
    $no = $no + 1;
    $varDiv = @($row['GROSSWEIGHT'] / $row['PROGRESSGROSS'] * 100);
    $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue("B$baris", $no)
            ->setCellValue("C$baris", $row['PROJECTNAME'])
            ->setCellValue("D$baris", $row['GROSSWEIGHT'])
            ->setCellValue("E$baris", $row['PROGRESSGROSS'])
            ->setCellValue("F$baris", number_format($varDiv, 2) . '%');


    $baris = $baris + 1;
    $SUMMARY_TOTAL_WEIGHT+=$row['GROSSWEIGHT'];
    $SUMMARY_TOTAL_PROGRESS+=$row['PROGRESSGROSS'];
}
