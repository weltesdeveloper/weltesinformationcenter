...
$objPHPExcel->setActiveSheetIndex(0)
        ->mergeCells('A8:N8')
        ->setCellValue('A8', "WEEKLY PROGRESS REPORT FOR GROSS WEIGHT");
...
$objPHPExcel->setActiveSheetIndex(0)
        ->mergeCells('B15:E15');
$objPHPExcel->getActiveSheet()->getStyle("A15:N15")->applyFromArray($stylebackground);
$objPHPExcel->getActiveSheet()->getStyle("A15:N15")->applyFromArray($styleArray2);

$today = date('w');
$sql = "SELECT STO.*,(SELECT SUM (GROSSWEIGHT)FROM SITE_TIER_ONE_VW STO WHERE PROJECTNO = '$jobName') TOTAL_PROJECT "
...
while ($row = oci_fetch_array($parse)) {
    ...
    $hasilLastWeek = 0.00;
    $sql1 = "SELECT  GET_PROCENTAGE_ALL (SUM (VSD.GROSS_ONSITE) * 5 / 100, SUM (VSD.GROSS_PREP) * 5 / 100, SUM (VSD.GROSS_ERECT) * 85 / 100, SUM (VSD.GROSS_QC) * 5 / 100) 
    ...
    $physiscalLastWeek = number_format($hasilLastWeek / $row['GROSSWEIGHT'] * 100, 2);

    //HITUNG PHISICAL THIS WEEK
    $hasilthisweek = 0.00;
    $sql2 = "SELECT  GET_PROCENTAGE_ALL (SUM (VSD.GROSS_ONSITE) * 5 / 100, SUM (VSD.GROSS_PREP) * 5 / 100, SUM (VSD.GROSS_ERECT) * 85 / 100, SUM (VSD.GROSS_QC) * 5 / 100) 
        AS TOTAL FROM VW_SHOW_DATA VSD
    ...
    $physiscalThisWeek = number_format($hasilthisweek / $row['GROSSWEIGHT'] * 100, 2);
    ...
    while ($row1 = oci_fetch_array($parsePerComponent)) {
        ...
        $persenOnsite = ($row1['GR_ONSITE'] / $row1['TOTAL_WEIGHT'] * 100);
        $persenPrep = $row1['GR_PREPARATION'] / $row1['TOTAL_WEIGHT'] * 100;
        $persenErection = $row1['GR_ERECTION'] / $row1['TOTAL_WEIGHT'] * 100;
        $persenQc = $row1['GR_QC'] / $row1['TOTAL_WEIGHT'] * 100;
        ...
        $sqlLastWeek = "SELECT NVL(SUM (GROSS_ONSITE),0) GROSS_ONSITE, 
                            NVL(SUM(GROSS_PREP) ,0) GROSS_PREP, 
                            NVL(SUM(GROSS_ERECT),0) GROSS_ERECT, 
                            NVL(SUM(GROSS_QC),0) GROSS_QC, 
                            COMP_TYPE
                                FROM VW_SHOW_DATA
        ...
        while ($row2 = oci_fetch_array($parseLastWeek)) {
            $persenOnsite = $row2['GROSS_ONSITE'] / $totat_weight * 100;
            $persenPrep = $row2['GROSS_PREP'] / $totat_weight * 100;
            $persenErection = $row2['GROSS_ERECT'] / $totat_weight * 100;
            $persenQc = $row2['GROSS_QC'] / $totat_weight * 100;
        ...
        $sqlThisWeek = "SELECT NVL(SUM (GROSS_ONSITE),0) GROSS_ONSITE, 
                            NVL(SUM(GROSS_PREP) ,0) GROSS_PREP, 
                            NVL(SUM(GROSS_ERECT),0) GROSS_ERECT, 
                            NVL(SUM(GROSS_QC),0) GROSS_QC, 
                            COMP_TYPE
...
$sqlWeightAll = "SELECT SUM (GROSSWEIGHT) NETT_WEIGHT FROM SITE_TIER_ONE_VW STO WHERE PROJECTNO = '$jobName'";
...
$sqlLastWeek = "SELECT GET_PROCENTAGE_ALL (
                SUM (VSD.GROSS_ONSITE) * 5 / 100,
                SUM (VSD.GROSS_PREP) * 5 / 100,
                SUM (VSD.GROSS_ERECT) * 85 / 100,
                SUM (VSD.GROSS_QC) * 5 / 100)
                AS TOTAL
...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue("F$baris", number_format($weightAll,2))
...
$objPHPExcel->getActiveSheet()->getStyle("B$baris:N$baris")->applyFromArray($styleArrayRight);
$objPHPExcel->getActiveSheet()->getStyle("B$baris:N$baris")->applyFromArray($styleBold);
...
header('Content-Disposition: attachment;filename="GROSSREPORT' . $jobName . '_' . $formattedDate . '.xls"');
