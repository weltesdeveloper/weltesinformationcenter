$(document).ready(function(){
	...
	var Modul = $("section#view_kat_product");
	listTabel();
	...
	Modul.find('form#formInput').bootstrapValidator({
		...
		var options = {
			type		: "POST",
			dataType	: 'json',
			url			: "pages/M-Kat-Product/model-kat-product.php",
			data		: {action:'commit'},
			...
			"columnDefs": [ 
				...
				},
				{
					...
					"render": function ( data, type, row, meta ) {
						...
						edit += "onclick='editData("+row.id_kat_product+")'><i class='fa fa-edit'></i></a> ";
						  
						var hapus = "<a title='Hapus Data' class='btn btn-xs btn-danger' ";
						hapus += "onclick='hapusData("+row.id_kat_product+")'><i class='fa fa-trash-o'></i></a> ";
						return edit + hapus;
					}
				}		
			]
...
function editData(idRow){
	...
	$.ajax({
		...
		success: function (json) {
			...
			Modul.find('input#id').val(json[0].id_kat_product);
			Modul.find('input#nama_kat').val(json[0].nama_kat_product);
