    ...
    if (isset($_POST['submit'])){
        ...
        if ($dataFinishing==0 and $dataBlast==0 and $dataPrimer==0 and $dataIntermd==0 ) {
          ...
          if ($dataBlast > 0){
              ...
              $blastUpdateParse = oci_parse($conn, "UPDATE PAINTING SET BLASTING = BLASTING+$dataBlast, BLASTING_PAINT_DATE = SYSDATE, BLASTING_PAINT_SIGN = :sign "
          ...
          if ($dataPrimer > 0){
              ...
              $SQLUpdateParse = oci_parse($conn, "UPDATE PAINTING SET PRIMER = PRIMER+$dataPrimer, PRIMER_PAINT_DATE = SYSDATE, PRIMER_PAINT_SIGN = :sign "
                      . " WHERE HEAD_MARK = :headmarkToUpdate AND ID = :idToUpdate");
          ...
          if ($dataIntermd > 0){
              ...
              $SQLUpdateParse = oci_parse($conn, "UPDATE PAINTING SET INTERMEDIATE = INTERMEDIATE+$dataIntermd, INTERMEDIATE_PAINT_DATE = SYSDATE, INTERMEDIATE_PAINT_SIGN = :sign "
                      . " WHERE HEAD_MARK = :headmarkToUpdate AND ID = :idToUpdate");
        ...
        if ($dataQCPass > 0) {
          // insert PAINT QC
            if ($FabPassQTY == ($ValueQCPASSFrst+$dataQCPass)) {
              ...
              $SQLUpdateParse = oci_parse($conn, "UPDATE PAINTING_QC SET 
                ...
                INTERMEDIATE_QC = INTERMEDIATE_QC+$dataQCPass, INTERMEDIATE_QC_DATE = SYSDATE, INTERMEDIATE_QC_SIGN = '$username', 
                FINISHING_QC = FINISHING_QC+$dataQCPass, FINISHING_QC_DATE = SYSDATE, FINISHING_QC_SIGN = '$username', 
                PAINT_QC_PASS = PAINT_QC_PASS+$dataQCPass
              WHERE HEAD_MARK = :headmarkToUpdate AND ID = :idToUpdate");
              $memo = "QCNOTPASS";
            }
          ...
          // update PAINT OPNAME
            $pnt_opn_PARSE = oci_parse($conn, "UPDATE PAINTING_OPN SET MEMO='NOTPASS' WHERE HEAD_MARK= :headmarkToUpdate AND ID= :idToUpdate AND MEMO='PASSED'");
            oci_bind_by_name($pnt_opn_PARSE, ":headmarkToUpdate", $HEAD_MARK);
            oci_bind_by_name($pnt_opn_PARSE, ":idToUpdate", $HM_ID);
            $pnt_opn_RES = oci_execute($pnt_opn_PARSE);
            if ($pnt_opn_RES){
                oci_commit($conn);
                // echo "<script>alert('QC PASS UPDATED');</script>";
            } else {
          ...
          //Insert OR Update PACKINGLIST
            if ($jmlHMPckList == 0) {
              ...
              # code...
              $updatePckListingSql = "UPDATE PREPACKING_LIST SET UNIT_QTY = UNIT_QTY+$dataQCPass, PACKING_STATUS='NP' ,ENTRY_DATE = SYSDATE ,ENTRY_SIGN = '$username' "
...
// SHOW IN ROW TABLE
    ...
    while ($row = oci_fetch_assoc($tableParse)){
     ...
     ?>
        ...
        <td><font size='2'><b><?php echo number_format($row["SURF"],2) ?></b></font></td>
        ...
        <td style="background-color: #EA6767;">
         ...
         <?php 
          ...
          $btn_opn_dis = 'disabled=""';
          $btn_opn_val = 'N/A';
          if ($row['PAINT_QC_PASS']<>0) {
            $btn_opn_dis = "";
            $btn_opn_val = 'Show';
          }
          ?>
          <td><button <?php echo $btn_opn_dis ?>  onclick="getHM_opn('<?php echo $row["HEAD_MARK"] ?>','<?php echo $row["ID"] ?>','<?php echo $row["SURF"] ?>','<?php echo $row["FAB_PASS_QTY"] ?>','<?php echo $row["SUBCONT_ID"] ?>');return false;" data-toggle="modal" data-target="#myModal" class="btn btn-info btn-sm" value="<?php echo $btn_opn_val ?>" name="opn_btn"><?php echo $btn_opn_val ?></button></td>
<?php
    }
