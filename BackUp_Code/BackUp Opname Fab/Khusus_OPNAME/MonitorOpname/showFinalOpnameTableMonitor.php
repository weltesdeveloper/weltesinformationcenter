    ...
    <tbody>
        ...
        while ($row = oci_fetch_array($finalOpnameParse)) {
            ...
            <tr id="rowsource<?php echo "$i"; ?>">
                ...
                <td><button type='button' class='btn btn-warning' onclick="printOpname('<?php echo "$i"; ?>');" id="printOpname<?php echo "$i"; ?>">PRINT</button></td>
            </tr>
            <?php
            $i++;
        }
...
<script>
    ...
    function detailRow(index) {
        ...
        if (row.child.isShown()) {
            ...
            $.ajax({
                type: 'GET',
                url: "MonitorOpname/opnameTableDetails.php",
                data: {opnameId: opnameId, prjName: prjName},
                beforeSend: function ()
                {
    ...
    function printOpname(param) {
        var opnameId = $('#opnId' + param).text().trim();
        var project_name = $('#prjName'+param).text().trim();
        var subcont = $('#sbctId'+param).text().trim();
