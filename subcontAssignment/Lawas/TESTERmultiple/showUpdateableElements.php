...
// $query = "SELECT HEAD_MARK FROM MASTER_DRAWING WHERE PROJECT_NAME = :projectName AND COMP_TYPE = :compType AND DWG_STATUS='ACTIVE'";
...
?>
    ...
    <?php
    $qtySql = "SELECT TOTAL_QTY AS UNIT_QTY FROM MASTER_DRAWING WHERE PROJECT_NAME = :PROJNAME AND COMP_TYPE = :COMP AND HEAD_MARK = :HM AND DWG_STATUS='ACTIVE'";
    ...
    <?php
    $assignedQtySql = "SELECT SUM(UNIT_QTY) AS ASSIGN_QTY FROM FABRICATION WHERE PROJECT_NAME = :PROJNAME AND HEAD_MARK = :HM";
    ...
    // echo "$availQty = $unitQty - $assignmentQty";
    ?>
